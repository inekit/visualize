...
const DEMO_ANIMATION: AnimationGroups = {
  base: { fall: 800, bounce: 250, magnet: 700 },
  bonus: { fall: 600, bounce: 150, magnet: 580 },
  demo: { fall: 1000, bounce: 200, magnet: 600 },
};
...
export const SimplePvPTest: React.FC = () => {
  ...
  const handleBallComplete = (ball: Ball, status: string) => {
    console.log(`Шарик ${ball.id} завершил анимацию:`, ball, status);
  };
  ...
  const startSinglePlayer = () => {
    ...
    setPlayers([TEST_PLAYERS[0]]);
    setBalls([TEST_BALLS[2], TEST_BALLS[3]]); // bonus ball
    setTimer(undefined);
  };
  ...
  const startRound = () => {
    setCurrentState('round');
    setPlayers(TEST_PLAYERS);
    setBalls(TEST_BALLS);
    setTimer(undefined);
  };

  // Обратный отсчет для multi-режима
  useEffect(() => {
    ...
    <div style={{ padding: '20px', background: '#f0f0f0', minHeight: '100vh' }}>
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ fontSize: '14px', color: '#666' }}>
          ...
          {balls.length > 0 && ` | Шариков: ${balls.length}`}
        </div>
      </div>
